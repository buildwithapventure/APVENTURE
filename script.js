...
(function () {
  ...
  function initPageLoader() {
    ...
    setTimeout(() => loader.classList.add("is-assembled"), 30);
    setTimeout(() => {
      loader.classList.add("is-launched");
      loader.classList.add("is-loaded");
    }, 1100);
    setTimeout(() => loader.classList.add("is-exiting"), 1750);
    setTimeout(() => {
      loader.classList.add("is-hidden");
  ...
  function initThemeToggle() {
    const STORAGE_KEY = "apventure-theme";
    ...
    function syncIcons(theme) {
      ...
      document.querySelectorAll(".icon-moon-m").forEach(function (el) { el.style.display = isDark ? "block" : "none"; });
      document.querySelectorAll(".icon-sun-m").forEach(function (el) { el.style.display = isDark ? "none" : "block"; });
    }
  ...
  /* ============================================================
     THREE.JS HERO SCENE — 1:1 port of HeroLaptopScene.tsx
     ============================================================ */

  function initHeroScene() {
    const canvas = document.getElementById("hero-canvas");
    const loadingEl = document.getElementById("hero-canvas-loading");
    ...
    const pointLight1 = new THREE.PointLight(0x2563eb, 1.2);
    pointLight1.position.set(3, 3, 3);
    scene.add(pointLight1);
    const pointLight2 = new THREE.PointLight(0xf97316, 1);
    pointLight2.position.set(-3, -2, 2);
    scene.add(pointLight2);
    const pointLight3 = new THREE.PointLight(0x8b5cf6, 0.8);
    pointLight3.position.set(0, 2, -2);
    scene.add(pointLight3);

    const laptopGroup = new THREE.Group();
    laptopGroup.rotation.set(0.18, -0.35, 0);
    ...
    baseMesh.position.set(0, -0.62, 0.15);
    laptopGroup.add(baseMesh);

    const deckMesh = new THREE.Mesh(
      new THREE.BoxGeometry(2.45, 0.02, 1.5),
      new THREE.MeshStandardMaterial({ color: 0x1e293b, metalness: 0.6, roughness: 0.4 })
    );
    deckMesh.position.set(0, -0.57, 0.1);
    laptopGroup.add(deckMesh);

    const screenGroup = new THREE.Group();
    screenGroup.position.set(0, 0.1, -0.62);
    screenGroup.rotation.set(-0.12, 0, 0);
    ...
    screenGroup.add(housingMesh);

    const screenGlowMaterial = new THREE.MeshBasicMaterial({ color: 0x1d4ed8, transparent: true, opacity: 0.55 });
    const screenGlowMesh = new THREE.Mesh(new THREE.PlaneGeometry(2.4, 1.4), screenGlowMaterial);
    screenGlowMesh.position.set(0, 0, 0.035);
    screenGroup.add(screenGlowMesh);

    const screenAccentMesh = new THREE.Mesh(
      new THREE.PlaneGeometry(2.4, 1.4),
      new THREE.MeshBasicMaterial({ color: 0x8b5cf6, transparent: true, opacity: 0.12 })
    );
    screenAccentMesh.position.set(0, 0, 0.04);
    screenGroup.add(screenAccentMesh);

    const planeGroup = new THREE.Group();
    planeGroup.scale.setScalar(0.16);
    ...
    const trailPoints = [];
    for (let i = 0; i < TRAIL_LENGTH; i++) trailPoints.push(new THREE.Vector3(0, 0, 0));
    ...
    function animate() {
      ...
      laptopGroup.position.y = Math.sin(t * 0.7) * 0.08;
      laptopGroup.rotation.y = THREE.MathUtils.lerp(laptopGroup.rotation.y, mouse.x * 0.25 - 0.35, 0.06);
      laptopGroup.rotation.x = THREE.MathUtils.lerp(laptopGroup.rotation.x, -mouse.y * 0.12 + 0.18, 0.06);
      screenGlowMaterial.opacity = 0.55 + Math.sin(t * 1.4) * 0.08;

      const loopT = (t * 0.35) % (Math.PI * 2);
      const x = Math.sin(loopT) * 1.6 + mouse.x * 0.3;
      const y = 0.55 + Math.sin(loopT * 2) * 0.3 + mouse.y * -0.15;
      const z = -0.4 + Math.cos(loopT) * 1.1;

      planeGroup.position.set(x, y, z);
      planeGroup.rotation.z = -Math.cos(loopT) * 0.4;
      planeGroup.rotation.y = loopT + Math.PI / 2;
      planeGroup.rotation.x = Math.sin(loopT * 2) * 0.2;
